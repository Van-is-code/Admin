                    ...
                    <div class="card shadow">
                        <div class="card-header py-3">
                            <p class="text-primary m-0 fw-bold">User</p>
                             <button id="tooltip" class="btn_add" type="button" onclick="addUser()"><img class="icon" width="30" height="30" src="./img/add.png"> <span class="tooltiptext">Add</span></button>
                        </div>
                        <div class="card-body">
                            <div class="row">
                            ...
                            <div class="table-responsive table mt-2" id="dataTable" role="grid" aria-describedby="dataTable_info">
                                <table class="table my-0" id="dataTable">
                                    ...
                                    if (isset($_GET['term'])) {
                                        ...
                                        $query = "SELECT * FROM user WHERE role = 'user' AND (user_id LIKE ? OR first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR phone LIKE ? OR password LIKE ?) LIMIT ?, ?";
                                        ...
                                        $sql = "SELECT * FROM user WHERE role = 'user' LIMIT ?, ?";
                                    ...
                                    if ($result->num_rows > 0) {
                                        ...
                                        while ($row = $result->fetch_assoc()) {
                                            ...
                                            echo '<td><img class="rounded-circle me-2" width="30" height="30" src="upload/user/' . $row["image"] . '">' . $row['first_name'] . ' ' . $row['last_name'] . '</td>';
                                    ...
                                    if (isset($_GET['term'])) {
                                        $searchTerm = $_GET['term'];
                                        $query = "SELECT COUNT(*) AS total FROM user WHERE role = 'user' AND (user_id LIKE ? OR first_name LIKE ? OR last_name LIKE ? OR email LIKE ? OR phone LIKE ? OR password LIKE ?)";
                                        $stmt = $conn->prepare($query);
                                        $searchTerm = "%$searchTerm%";
